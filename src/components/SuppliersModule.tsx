    ...
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
        <div className="bg-white p-4 lg:p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          <p className="text-2xl lg:text-3xl font-bold text-indigo-600">{suppliers.length}</p>
          <p className="text-xs lg:text-sm text-gray-500 mt-1">Fournisseurs actifs</p>
        </div>
        <div className="bg-white p-4 lg:p-6 rounded-lg shadow-sm border border-gray-200">
          <h3 className="text-base lg:text-lg font-semibold text-gray-900 mb-2">Commandes ce mois</h3>
          <p className="text-2xl lg:text-3xl font-bold text-green-600">35</p>
          <p className="text-xs lg:text-sm text-gray-500 mt-1">+12% vs mois dernier</p>
        </div>
        <div className="bg-white p-4 lg:p-6 rounded-lg shadow-sm border border-gray-200">
          <h3 className="text-base lg:text-lg font-semibold text-gray-900 mb-2">Montant total</h3>
          <p className="text-2xl lg:text-3xl font-bold text-orange-600">€100,200</p>
          ...
          {filteredSuppliers.map((supplier) => (
            <div key={supplier.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              ...
              <div className="border-t pt-3">
                <p className="text-xs text-gray-500 mb-3">Dernière commande: {supplier.lastOrder}</p>
                <div className="flex flex-col sm:flex-row gap-2">
                  <Button variant="outline" size="sm" className="flex-1 text-xs">Voir</Button>
                  <Button variant="outline" size="sm" className="flex-1 text-xs">Commander</Button>
                </div>
              </div>
            </div>
          ))}
